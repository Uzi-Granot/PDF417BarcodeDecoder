...
internal sealed class SampleGrabberHelper : ISampleGrabberCB, IDisposable
	...
	public Bitmap SnapshotNextFrame()
		{
		if(m_SampleGrabber == null) throw new Exception("SampleGrabber was not initialized");

		// capture image
		IntPtr ip = GetNextFrame();

		if(ip == IntPtr.Zero) throw new Exception("Can not snap next frame");
	...
	public Bitmap SnapshotCurrentFrame()
		...
		if(!m_bBufferSamplesOfCurrentFrame) throw new Exception("SampleGrabberHelper was created without buffering-mode (buffer of current frame)");
	...
	private IntPtr GetNextFrame()
		...
		try
			...
			// Start waiting
			if(!m_PictureReady.WaitOne(5000, false)) throw new Exception("Timeout while waiting to get a snapshot");
			}
		catch
			{
